...
- **Header styling** — row 1 is frozen with an `#F5A04C` background, and the header labels are prettified (underscores → spaces, title-cased, with known acronyms like `URL`/`ID` fully capitalised), e.g. `change_flag` → `Change Flag`, `image_url` → `Image URL`. Reapplied every run.
